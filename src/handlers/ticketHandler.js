...
async function handleBuy(interaction, publisherId, client) {
    ...
    const existing = await Ticket.findOne({ buyerId: interaction.user.id, sellerId: publisherId, closed: false });
    if (existing) {
        return interaction.reply({ content: "**لديك تذكرة مفتوحة مع هذا البائع بالفعل.**", ephemeral: true });
    }
    ...
    const channel = await guild.channels.create({
        ...
        permissionOverwrites: [
            ...
            { id: publisherId, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages] },
        ],
    });
    ...
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(new TextDisplayBuilder().setContent(`<@${interaction.user.id}> <@${publisherId}>`))
